...
for t in range(mask.shape[0]):   
    ...
    temp_bounds = np.max(temp_bounds, axis=0)
    bounds.append(temp_bounds)
bounds = np.array(bounds)
    
# viewer = napari.Viewer()
# viewer.add_image(temp_bounds)
# viewer.add_image(np.array(bounds))
...
# Normalize data
raw = norm_data(raw)
bounds = norm_data(bounds)

# Split data
raw_trn, mask_trn, raw_val, mask_val = split_data(raw, bounds, 0.33)
